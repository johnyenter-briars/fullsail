...
public class FullSailClient
{
    ...
    public async Task<List<TorrentSearchResult>> GetTorrentSearchResults(string query, TorrentSearchWebsite searchWebsite = TorrentSearchWebsite.solid)
    {
        ...
        return await FullSailRequest<List<TorrentSearchResult>>($"search/magnetlink/{searchWebsite}/{encodedQuery}", HttpMethod.Get);
    }
    public async Task<List<SubtitleSearchResult>> GetSubtitleSearchResults(string query, SubtitleSearchWebsite searchWebsite = SubtitleSearchWebsite.opensubtitles)
    {
        var encodedQuery = query.Replace(" ", "+").ToString();

        return await FullSailRequest<List<SubtitleSearchResult>>($"search/subtitle/{searchWebsite}/{encodedQuery}", HttpMethod.Get);
    }
    public async Task<FullSailResponse> DownloadSubtitle(SubtitleSearchResult subtitleSearchResult, SubtitleSearchWebsite searchWebsite)
    {
        var encodedLink = Uri.EscapeDataString(subtitleSearchResult.FullLink);
        var url = $"subtitle/download/{searchWebsite}/{encodedLink}";
        return await FullSailRequest<FullSailResponse>(url, HttpMethod.Post);
    }
    ...
    public async Task<UpdateFileMediaSystemResponse> SendFile(string fileNamePlusDirectory, string computerDestination)
    {
        var body = new UpdateFileMediaSystemRequest
        {
            ...
            ComputerDestination = computerDestination,
        };
